...
export const createPost = createAsyncThunk('posts/createpost', async (post) => {
    const res = await fetch(url + `/posts`, { method: 'POST', body: JSON.stringify(post) })
    return await res.json()
})
...
const Posts = createSlice({
    ...
    reducers: {
        ...
        addPost: (state, active) => {
            state.posts = [active.payload, ...state.posts]
        },
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(getPosts.fulfilled, (state, action) => {
                ...
                state.postsLength = action.payload.length;
            })
            ...
            .addCase(createPost.fulfilled, (state, action) => {
                console.log(`Post ${action.meta.arg.id} was created!`)
...
export const { removePost, findPost, clearComments, addPost, falseWasCreated } = Posts.actions;